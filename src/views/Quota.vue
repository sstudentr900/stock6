...
<template>
  ...
  <div class="flex gap-6 mb-6">
    <div class="max-w-96 w-full">
      <!-- class="flex flex-col gap-6 sticky top-[90px] bg-white" -->
      <SharedItem2 title="交易參數" class="sticky top-[90px] bg-white">
        ...
        <template v-slot:content>
          ...
          <div class="flex flex-col gap-1 mt-1">
            <button class="p-3 bg-blue-500 text-white" @click="send">送出</button>
          </div>
        </template>
      </SharedItem2>
    </div>
        ...
        <template v-slot:content>
          <div class="">
            ...
            <div class="text-sm text-gray-500">
              <div class="border-t border-solid border-gray-200 w-full flex items-center" v-for="(list,index) in state.lists" :key='index' :class="{'bg-blue-500/10':index==state.lists.length-1}">
          ...
          <div class="mt-4">
            ...
            <div class="text-sm text-gray-500">
              <div class="border-t border-solid border-gray-200 w-full flex items-center" v-for="(list,index) in state.lists2" :key='index' :class="{'bg-blue-500/10':index==state.lists2.length-1}">
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.year }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.price.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.number.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.fluctuation.toLocaleString()}}%</div>
      ...
      <SharedItem title="試算除息不再投入結果">
        ...
        <template v-slot:content>
          <div class="">
            ...
            <div class="text-sm text-gray-500">
              <div class="border-t border-solid border-gray-200 w-full flex items-center" v-for="(list,index) in state.lists3" :key='index' :class="{'bg-blue-500/10':index==state.lists3.length-1}">
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.year }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.price.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.number.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.fluctuation.toLocaleString()}}%</div>
          ...
          <div class="mt-4">
            ...
            <div class="text-sm text-gray-500">
              <div class="border-t border-solid border-gray-200 w-full flex items-center" v-for="(list,index) in state.lists4" :key='index' :class="{'bg-blue-500/10':index==state.lists4.length-1}">
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.year }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.price.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.number.toLocaleString() }}</div>
                <div class="flex-1 flex justify-center px-3 py-3">{{ list.fluctuation.toLocaleString()}}%</div>
        ...
        <template v-slot:content>
          ...
          <div class="text-sm text-gray-500">
            <div class="border-t border-solid border-gray-200 w-full flex items-center">
              <div class="flex w-40 px-6 py-3">有無配息</div>
              <div class="flex flex-1 px-6 py-3">從最終市值來看，有無配息，並沒有改變最終報酬。</div>
            </div>
            <div class="border-t border-solid border-gray-200 w-full flex items-center">
              <div class="flex w-40 px-6 py-3">每年提領</div>
              <div class="flex flex-1 px-6 py-3">從每年提領生活費來看，領股息跟自己變賣股票，也沒有改變最終報酬。</div>
            </div>
            <div class="border-t border-solid border-gray-200 w-full flex items-center">
              <div class="flex w-40 px-6 py-3">股息增加</div>
              <div class="flex flex-1 px-6 py-3">即使用股息增加很多股數，也不影響最終報酬</div>
            </div>
            <div class="border-t border-solid border-gray-200 w-full flex items-center">
              <div class="flex w-40 px-6 py-3">零成本</div>
              <div class="flex flex-1 px-6 py-3">現在成本殖利率還高達幾十趴，幹麻賣？雖然很阿Ｑ，但你只看成本的精神，讓他們長期持有一檔股票數年，甚至十年以上。</div>
            </div>
            <div class="border-t border-solid border-gray-200 w-full flex items-center">
              <div class="flex w-40 px-6 py-3">參考連結</div>
              <div class="flex flex-1 px-6 py-3">
